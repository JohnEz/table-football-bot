...
const getIntialAddInputs = (session, args, next) => {
	...
	fetch('http://localhost:53167/api/players').then(players => {
		session.dialogData.playerDocs = players;
		next();
	});
}

const askForPlayerOne = (session, results, next) => {
	...
	if (playersFound.length > 1) {
		playersFound.forEach((player, index, array) => {
...
const askForPlayerTwo = (session, results, next) => {
	let playerDocs = session.dialogData.playerDocs;
	let result = session.dialogData.result;
	let validation = session.dialogData.validation;
	let foundString = '';
...
const getPlayer = (player, failPrompt) => {
...
const validatePlayers = (session, results, next) => {
	let result = session.dialogData.result;
	//now we have the final player docs, do validation
	if (session.dialogData.validation.passed) {
		controller.getMatches(result.p1, result.p2, matches => {
...
const askForFirstScore = (session, results, next) => {
	let playerDocs = session.dialogData.playerDocs;
	let result = session.dialogData.result;
	let validation = session.dialogData.validation;
...
const askForSecondScore = (session, results, next) => {
	let result = session.dialogData.result;
	let validation = session.dialogData.validation;
...
const validateScores = (session, results, next) => {
...
const respondFinalResult = (session, results) => {
	let result = session.dialogData.result;
	let validation = session.dialogData.validation;
	if (result.p1 && result.p2 && result.s1 !== null && result.s2 !== null && validation.passed) {
		controller.submitResult(result.p1, result.p2, result.s1, result.s2, result.win, result.loss, result.draw, session.dialogData.match, (message, endResult) => {
...
const outputMatch = (session, matches, user) => {
...
const outputMatches = (session, matches, includesUser) => {
	let matchesString = ''
